...
// {   if(singleQueue==NULL||mergeQueue==NULL){return NULL;}
    ...
    if(mergeQueue.empty()&&(!singleQueue.empty())){
      TreeNode *temp3=singleQueue.front();
      singleQueue.pop();
      return temp3;
    }
    if((mergeQueue.front())->freq<(singleQueue.front()->freq)){
      TreeNode *temp1=mergeQueue.front();
      mergeQueue.pop();
      return temp1;
    }
    else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    // for(std::vector<Frequency>::iterator it=frequencies.begin();it!=frequencies.end();it++){
    //   TreeNode* leaf=new TreeNode(*it)
    //   singleQueue.push(leaf);
    // }
    if(frequencies.empty()){return;}
    for(Frequency frequency:frequencies){
      TreeNode* leaf=new TreeNode(frequency);
      singleQueue.push(leaf);
    }
    ...
    while(!(singleQueue.size()+mergeQueue.size()==1))
    {
      ...
      int internalFreq=n1->freq.getFrequency()+n2->freq.getFrequency();
      TreeNode* internal=new TreeNode(internalFreq);
      internal->left=n1;
      internal->right=n2;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile,TreeNode *current){
  ...
  if(b==0){
    ...
    internal->left=readTree(bfile,internal->left);
    internal->right=readTree(bfile,internal->right);
    return internal;
  }
  return NULL;
}

...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1.begin(), word1.end(), upperWord1.begin(), ::toupper);
    std::transform(word2.begin(), word2.end(), upperWord2.begin(), ::toupper);
    if(dict.find(upperWord1)==dict.end()||dict.find(upperWord2)==dict.end()){
      return false;
    }
    if(dict.find(upperWord1)->second.size()!=dict.find(upperWord2)->second.size()){
      return false;
    }
    for(size_t i=0;i<dict.find(upperWord1)->second.size();i++){
      if((dict.find(upperWord1)->second)[i]!=(dict.find(upperWord2)->second)[i]){
        return false;
      }
    }
    return true;

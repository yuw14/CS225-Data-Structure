...
/**
 * Returns an image with a spotlight centered at  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      HSLAPixel & pixel = image.getPixe (`centerX`, `centerY`).
 *
 ...
 * its luminance is decreased by 2.5% (0.975x its original value).  At a  pixel.s = 0;
...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      else{pixel.l=(1-0.005*dis)*pixel.l;}
    }
  }
  return image;

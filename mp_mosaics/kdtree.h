...
class KDTree
{
  ...
  public:
    ...
    int partition(vector<Point<Dim>>& points, int left, int right, unsigned pivotIndex, int dimension);

    Point<Dim> quickSelect(vector<Point<Dim>>& points, int left, int right, int k, int dimension);

    KDTreeNode* ctor(vector<Point<Dim>>& points, int dimension, unsigned left, unsigned right);

    KDTree<Dim>::KDTreeNode* copy(KDTreeNode* subRoot);

    void deleteTree(KDTreeNode* subRoot);

    void findHelper(KDTreeNode* curr_node, Point<Dim> &curr_best, const Point<Dim>& query,int dimension )const;
